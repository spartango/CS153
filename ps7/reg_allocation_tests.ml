...
let test_is_simplified (num_regs: int) (g: interfere_graph) : unit -> bool =
    (fun () -> 
         is_simplified 
             num_regs
             (extract_igraph (simplify num_regs (initial_reduction_state g))))


let mk_simplify_test_graph (g: interfere_graph) (regs: int) (exp: interfere_graph) (name: string) =
    mk_generic_equals_test IGNodeSet.equal (fun () -> let s = simplify regs (initial_reduction_state g) in s.reduce_igraph) exp igraph2str name
